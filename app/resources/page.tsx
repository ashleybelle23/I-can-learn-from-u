  ...
  return (
    <div className="bg-[#fdf5f8] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
          ...
          <div className="space-y-8">
            {additionalResources.map((category) => (
              <div key={category.category}>
                <h3 className="text-sm font-semibold text-[#6a3858] mb-4">{category.category}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {category.resources.map((resource) => (
                    <a
                      ...
                      className="block p-5 rounded-xl border border-[#e8b8cc]/60 bg-[#fce8f0]/40 hover:bg-[#fce8f0] hover:border-[#e8b8cc] transition-all group"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <h4 className="text-sm font-semibold text-[#1e1428] group-hover:text-[#e8b030] transition-colors mb-1">
                            {resource.name}
                          </h4>
                          <p className="text-xs text-[#b890a0] leading-relaxed">{resource.description}</p>
                        </div>
                        <svg className="w-4 h-4 text-[#b890a0] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <section id="guidelines">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {communityGuidelines.map((guide) => (
              <div
                key={guide.title}
                className="p-5 rounded-2xl border border-[#e8b8cc]/60 bg-[#fce8f0]/40"
              >
                <h3 className="text-sm font-semibold text-[#6a3858] mb-2">{guide.title}</h3>
                <p className="text-xs text-[#48c8b0] leading-relaxed">{guide.body}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Report section */}
        <section id="report" className="rounded-2xl border border-[#e8b8cc]/60 bg-[#fce8f0]/40 p-8">
          <h2 className="text-xl font-bold text-[#1e1428] mb-3">Report harmful content</h2>
          ...
          <p className="text-xs text-[#b890a0] mt-4">
