  ...
  return (
    <div className="bg-[#07051a] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
        {/* Trust levels */}
        <div>
          <p className="text-xs font-semibold tracking-[0.2em] uppercase text-[#c89040] mb-8">
            Trust levels
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div>
            <p className="text-xs font-semibold tracking-[0.2em] uppercase text-[#c89040] mb-6">
              How to earn trust
            </p>
            <div className="space-y-3">
              {earnPointsExamples.map((item) => (
                <div
                  key={item.action}
                  className="flex items-center justify-between p-4 rounded-xl border border-[#3a2d78]/60 bg-[#130f30]/40"
                >
                  <p className="text-sm text-[#a898d8] leading-snug">{item.action}</p>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="rounded-2xl border border-[#3a2d78]/60 bg-[#130f30]/40 p-6 space-y-3">
                {[
                  ...
                  <div key={item} className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-[#2cd4f0]">{item}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="p-6 rounded-2xl border border-[#c89040]/20 bg-[#c89040]/5">
              <p className="text-sm font-semibold text-[#c89040] mb-3">
                A note on this system
              </p>
              <p className="text-sm text-[#2cd4f0] leading-relaxed">
        ...
        {/* Badges */}
        <div>
          <p className="text-xs font-semibold tracking-[0.2em] uppercase text-[#c89040] mb-2">
            Community badges
          </p>
          <p className="text-sm text-[#6858a8] mb-8 max-w-2xl">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {mockUsers.map((user) => (
              <div
                key={user.id}
                className="rounded-2xl border border-[#3a2d78]/60 bg-[#130f30]/40 p-5"
              >
                <div className="flex items-start gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full bg-[#1a1640] flex items-center justify-center text-base font-semibold text-[#a898d8] flex-shrink-0">
                    {user.name.charAt(0)}
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-semibold text-[#e8e4f8]">{user.name}</span>
                    </div>
                    <TrustBadge level={user.trustLevel} />
                  </div>
                </div>

                <p className="text-xs text-[#2cd4f0] leading-relaxed mb-4">{user.bio}</p>

                <div className="grid grid-cols-2 gap-3 text-center">
                  <div className="p-2 rounded-lg bg-[#07051a]/60 border border-[#3a2d78]/30">
                    <p className="text-base font-bold text-[#c89040]">{user.points.toLocaleString()}</p>
                    <p className="text-xs text-[#6858a8]">points</p>
                  </div>
                  <div className="p-2 rounded-lg bg-[#07051a]/60 border border-[#3a2d78]/30">
                    <p className="text-base font-bold text-[#a898d8]">{user.contributions}</p>
                    <p className="text-xs text-[#6858a8]">contributions</p>
                  </div>
                </div>
                ...
                {user.badges.length > 0 && (
                  <div className="flex items-center gap-1.5 mt-3 flex-wrap">
                    {user.badges.slice(0, 4).map((badge) => (
                      <span
                        key={badge.id}
                        className="w-7 h-7 rounded-full bg-[#1a1640] flex items-center justify-center text-sm"
                        title={badge.name}
                      >
                        {badge.icon}
                      </span>
                    ...
                    {user.badges.length > 4 && (
                      <span className="text-xs text-[#6858a8]">+{user.badges.length - 4}</span>
                    )}
                  </div>
                )}
              </div>
