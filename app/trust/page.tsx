  ...
  return (
    <div className="bg-[#09071c] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-20">
        {/* Trust levels */}
        <div>
          <p className="text-xs font-semibold tracking-[0.2em] uppercase text-[#d4a843] mb-8">
            Trust levels
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div>
            ...
            <div className="space-y-3">
              {earnPointsExamples.map((item) => (
                <div
                  key={item.action}
                  className="flex items-center justify-between p-4 rounded-xl border border-[#3c3070]/60 bg-[#181440]/40"
                >
                  <p className="text-sm text-[#c8b8f0] leading-snug">{item.action}</p>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="rounded-2xl border border-[#3c3070]/60 bg-[#181440]/40 p-6 space-y-3">
                {[
                  ...
                  <div key={item} className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-[#8aaa74]">{item}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="p-6 rounded-2xl border border-[#d4a843]/20 bg-[#d4a843]/5">
              <p className="text-sm font-semibold text-[#d4a843] mb-3">
                A note on this system
              </p>
              <p className="text-sm text-[#8aaa74] leading-relaxed">
        ...
        {/* Badges */}
        <div>
          <p className="text-xs font-semibold tracking-[0.2em] uppercase text-[#d4a843] mb-2">
            Community badges
          </p>
          <p className="text-sm text-[#9080c0] mb-8 max-w-2xl">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {mockUsers.map((user) => (
              <div
                key={user.id}
                className="rounded-2xl border border-[#3c3070]/60 bg-[#181440]/40 p-5"
              >
                <div className="flex items-start gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full bg-[#201c48] flex items-center justify-center text-base font-semibold text-[#c8b8f0] flex-shrink-0">
                    {user.name.charAt(0)}
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-semibold text-[#ede8f8]">{user.name}</span>
                    </div>
                    <TrustBadge level={user.trustLevel} />
                  </div>
                </div>

                <p className="text-xs text-[#8aaa74] leading-relaxed mb-4">{user.bio}</p>

                <div className="grid grid-cols-2 gap-3 text-center">
                  <div className="p-2 rounded-lg bg-[#09071c]/60 border border-[#3c3070]/30">
                    <p className="text-base font-bold text-[#d4a843]">{user.points.toLocaleString()}</p>
                    <p className="text-xs text-[#9080c0]">points</p>
                  </div>
                  <div className="p-2 rounded-lg bg-[#09071c]/60 border border-[#3c3070]/30">
                    <p className="text-base font-bold text-[#c8b8f0]">{user.contributions}</p>
                    <p className="text-xs text-[#9080c0]">contributions</p>
                  </div>
                </div>
                ...
                {user.badges.length > 0 && (
                  <div className="flex items-center gap-1.5 mt-3 flex-wrap">
                    {user.badges.slice(0, 4).map((badge) => (
                      <span
                        key={badge.id}
                        className="w-7 h-7 rounded-full bg-[#201c48] flex items-center justify-center text-sm"
                        title={badge.name}
                      >
                        {badge.icon}
                      </span>
                    ...
                    {user.badges.length > 4 && (
                      <span className="text-xs text-[#9080c0]">+{user.badges.length - 4}</span>
                    )}
                  </div>
                )}
              </div>
