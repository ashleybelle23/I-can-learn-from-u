...
export function Footer() {
  return (
    <footer className="border-t border-[#3c3070]/60 bg-[#f4ece0] mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          {/* Brand */}
          <div className="md:col-span-1">
            <div className="flex items-center gap-1 mb-4">
              <span className="text-[#d4a843] text-lg font-bold">I Can Learn</span>
              <span className="text-[#8aaa74] text-lg font-light">From You</span>
            </div>
            <p className="text-[#9080c0] text-sm leading-relaxed">
              A living community built on the belief that every person carries wisdom worth sharing and a story worth hearing.
            </p>
            <div className="mt-6 p-3 rounded-xl bg-[#181440]/60 border border-[#3c3070]/40">
          ...
          {Object.entries(footerLinks).map(([section, links]) => (
            <div key={section}>
              <h3 className="text-xs font-semibold tracking-[0.15em] uppercase text-[#d4a843] mb-4">
                {section}
              </h3>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-[#9080c0] hover:text-[#c8b8f0] transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        {/* Bottom bar */}
        <div className="mt-12 pt-8 border-t border-[#3c3070]/40 flex flex-col md:flex-row justify-between items-center gap-4">
