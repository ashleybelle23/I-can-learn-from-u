    ...
    <div className="rounded-2xl border border-red-800/40 bg-red-950/20 p-5 space-y-4">
      ...
      <div className="space-y-2.5">
        {emergencyResources.slice(0, 3).map((resource) => (
          <div
            key={resource.name}
            className="flex items-center justify-between p-3 rounded-xl bg-[#09071c]/60 border border-red-900/30 gap-4"
          >
            <div className="min-w-0">
              <p className="text-xs font-semibold text-red-200">{resource.name}</p>
              <p className="text-xs text-[#9080c0] mt-0.5 truncate">{resource.hours}</p>
            </div>
