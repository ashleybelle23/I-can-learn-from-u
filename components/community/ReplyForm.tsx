...
export function ReplyForm({ postId }: { postId: string }) {
  ...
  if (submitted) {
    return (
      <div className="rounded-2xl border border-[#231890]/20 bg-[#231890]/5 p-5 text-center">
        <p className="text-sm text-[#2a1a70]">Your response has been shared. Thank you for showing up.</p>
        <button
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="reply-body" className="block text-sm font-medium text-[#2a1a70] mb-2">
          Offer your support
        </label>
        <textarea
          id="reply-body"
      ...
      <div className="flex gap-3">
        ...
        <button
          type="button"
          onClick={() => setBody('')}
          className="px-4 py-3 rounded-xl text-sm text-[#3a2a80] border border-[#ffffff] hover:border-[#ffffff] hover:text-[#2a1a70] transition-colors"
        >
          Clear
        </button>
      </div>
